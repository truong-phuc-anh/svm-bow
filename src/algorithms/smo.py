...
class SMO:
    ...
    kernel : object
        ...
        the kernel has to have a function name "apply(x1, x2)"

    use_linear_optim : bool, optional (default=True)
        Whether use linear optimization or not.
    """
    def __init__(self, C=1.0, tol=1e-4, kernel=kernels.LinearKernel(), use_linear_optim=True):
        self.kernel = kernel
        self.C = C
        self.tol = tol
    ...
    def output(self, i):
        if self.use_linear_optim:
            ...
            return np.sum([self.alphas[j] * self.y[j]
                           * self.kernel.apply(self.X[j], self.X[i])
    ...
    def take_step(self, i1, i2):
        ...
        k11 = self.kernel.apply(X1, X1)
        k12 = self.kernel.apply(X1, self.X[i2])
        k22 = self.kernel.apply(self.X[i2], self.X[i2])
        ...
        for i in range(self.m):
            if 0 < self.alphas[i] < self.C:
                self.errors[i] += delta1 * self.kernel.apply(X1, self.X[i]) + \
                                  delta2 * self.kernel.apply(self.X2, self.X[i]) \
                                  - delta_b
    ...
    def get_probability_scores(self, X, method='sigmod'):
        """
